...
OUT = "bench/results/avg_latency_per_listener.png"
...
def main():
    ...
    for r in runs:
        ...
        val = lat.get("avg_per_listener_mean_ms", None)
        if val is None:
            val = ((lat.get("summary", {}) or {}).get("mean_ms", 0.0))
        data.setdefault(fanout, {})[n] = float(val or 0.0)

    fanouts = sorted(data.keys())
    xs = sorted({n for d in data.values() for n in d.keys()})
    if not fanouts or not xs:
        raise SystemExit("No successful runs found in system_results.json")
    ...
    ax.set_title("Average latency per listener")
